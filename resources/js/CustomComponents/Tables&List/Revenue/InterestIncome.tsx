...
import {
  ...
  const InterestIncome = (props: Props) => {
    const { accounts } = props
    const [data, setData] = useState<Account[]>(accounts)
  
    // GET ALL ACCOUNT TYPE Account Receivable
    useEffect(() => {
      const filteredData = accounts.filter((a: Account) => a.account_sub_id === 19) // 19 HERE MEANS InterestIncome
      ...
      <Command className="rounded-lg border shadow-md md:min-w-[450px]" >
        ...
        <CommandList>
          ...
          <CommandGroup heading="Account List">
            {
              data.map((dt: Account, index: number) => <CommandItem className="flex gap-x-3">
